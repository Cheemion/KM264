...
class NalUnit {
public:
	virtual	int getNextByte(byte& holder) { //0 means end
...
public:
	...
	void writeNAL(std::ofstream& os) {
		...
		int bit_num;
		//8 bits
		while((bit_num = nal_unit_p->getNextByte(b2)) == 8) {
...
public:
	...
	byte constraint_set0_flag; // u(1)
	byte constraint_set1_flag; // u(1)
	byte constraint_set2_flag; // u(1)
	byte constraint_set3_flag; // u(1)
	byte reserved_zero_4bits = 0; //u(4)
	byte level_idc = CONFIGURE::LEVEL_IDC; // level_idc shall be set equal to a value of ten times the level number specified in Table A - 1
	virtual	int getNextByte(byte& holder) { //0 means end
		static int i = 0;
		while(i != 5) {
			holder = 0;
...
void writeNALSequences(std::vector<NalContainer> nals, std::ofstream& os) {
	if(CONFIGURE::OUTPUT_FILE_MODE != "AnnexB")
