...
int main() {
	...
	SliceWithoutPartition slice2 = SliceWithoutPartition::generateBaselineSliceWithoutPartition(&pps,&sps, SLICE_TYPE::I, 1, NAL_UNIT_TYPE::CODEC_SLICE_OF_AN_IDR_PICTURE_SLICE_LAYER_WITHOUT_PARTITIONING_RBSP);
	SliceWithoutPartition slice3 = SliceWithoutPartition::generateBaselineSliceWithoutPartition(&pps,&sps, SLICE_TYPE::I, 2, NAL_UNIT_TYPE::CODEC_SLICE_OF_AN_IDR_PICTURE_SLICE_LAYER_WITHOUT_PARTITIONING_RBSP);
	for(int i = 0; i < CONFIGURE::HEIGHT * CONFIGURE::WIDTH / 256; i++) {
		slice1.addMacroBlock(&mbs[i]);
		slice2.addMacroBlock(&mbs[i]);
		slice3.addMacroBlock(&mbs[i]);
	}
